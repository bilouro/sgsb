...
class PrestacaoServicoPagamentoList(FormView):
    ...
    template_name = 'cadastro/cbv/prestacao_servico_pagamento.html'
    ...
    def form_valid(self, form):
        ...
        """
        pss_selected_list = None
        psc_selected_list = None
        ...
        if self.request.POST.get('Pagar', None):
            ...
            if form.is_valid():
                ...
                if ret_code == Pagamento.REALIZAR_PAGAMENTO_SUCESSO:
                    ...
                    messages.add_message(self.request, messages.ERROR, 'Houve divergencia, algum dos itens já foi pago')
        ...
        elif self.request.POST.get('Historico', None):
            return redirect("/cadastro/cliente/%s/pagamento/historico" % cliente.id)
        ...
        return render_to_response('cadastro/cbv/prestacao_servico_pagamento.html', {
            'form': form,
            'pss_list': pss_list,
            'psc_list': psc_list,
            'pss_selected_list': pss_selected_list,
            'psc_selected_list': psc_selected_list,
